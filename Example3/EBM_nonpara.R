...
################################################################################
pV_dens <- function(x, samples, adj){
  y <- try(density(samples, bw = "nrd", adjust = adj, 
                   kernel = "gaussian", n=length(xxx), from=-100, to=100))
  if(y[1]=="Error in density.default(samples, bw = \"nrd\", adjust = adj, kernel = \"gaussian\",  : \n  'bw' is not positive.\n"){
    y <- density(samples, bw = 10, adjust = adj, 
                 kernel = "gaussian", n=length(xxx), from=-100, to=100)
  }
  return(y$y)
}
...
out_old = U_fct(ZS_old, ZR_old, data) + V[abs(u-xxx)==min(abs(u-xxx))]

for(t in 1:nr_samples){
  
  ZS_new = sqrt(1-jr^2)*ZS_old + jr*rnorm(1)
  ZR_new = sqrt(1-(0.7*jr)^2)*ZR_old + 0.7*jr*rnorm(nr_R)
  
  u <- R_fct(ZS_new, ZR_new)
  out_new = U_fct(ZS_new, ZR_new, data) + V[abs(u-xxx)==min(abs(u-xxx))]
...
# ddd <- pV_dens(xxx, samples2,adj)
  
par(mfrow=c(3,2))
x <- seq(-100,100,0.05)
plot(xxx,V, type='l', xlab='r', ylab='', ylim=c(-20,20),xlim=c(-20,30), lwd=4, cex.main=1.5, main='V(r)', cex.lab=1.5)
ax <- seq(-100,1000,length.out=800)
hist(samples1, breaks=ax, xlab='r', ylab='', main='Samples p(r) and pV(r)', ylim=c(0,10), cex.main=1.5, cex.lab=1.5, xlim=c(-20,30))
hist(samples2, breaks=ax,add=TRUE, col='cornflowerblue')
# plot(xxx,ddd,'l', xlim=c(-20,30), col='red', ylim=c(0,0.2))
# lines(xxx,dgev(xxx,p_m,p_sd, shape=sh),xlim=c(-20,30),ylim=c(0,0.2))
#plot(loss, xlim=c(1,nr_tt), ylim=c(-10,10), xlab='Iteration', ylab='', cex.main=1.5, cex.lab=1.5, main='Loss')
abline(h=0)
plot(prob, log="y", xlim=c(1,nr_tt), ylim=c(10^(-7),10^(-0)), xlab='Iteration', ylab='', cex.main=1.5, cex.lab=1.5, main='Risk probability')
abline(h=2.1e-05)
...
if(learn_rate > learn_rate_end){
  learn_rate <- learn_rate*exp(-0.003*it)
}
