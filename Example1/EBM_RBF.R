...
ff <- function(x) {
  ...
  if(p_dist=='gumbel'){
    a <- - rbf(x) - log(dgumbel(x, p_m, p_sd))
  }
  if(p_dist=='gev'){
...
mome <- c(0,0.5,0.9)
conf <- matrix(0,20,3)
forward_mean <- matrix(0,20,1)
nr_optim <- matrix(0,20,2)
indd <- 1
...
centers <- seq(-80,120,length.out=500)
eps <- 1
...
learn_rate_end <- 0
momentum <- mome[mom]
velocity <- seq(0,0,length.out = length(centers))
...
u <- R_fct(sigma_old)
out_old = U_fct(sigma_old, J, y_true, obs_noise, prior_mean,prior_sd) + rbf(u)
...
for(t in 1:nr_samples){
  ...
  u <- R_fct(sigma_new)
  out_new = U_fct(sigma_new, J, y_true, obs_noise, prior_mean,prior_sd) + rbf(u)
...
ax <- seq(-20,2000,length.out=2000)
hist(samples1, breaks=ax, xlab='r', ylab='', main='Samples p(r) and pV(r)', xlim=c(0,50), ylim=c(0,20), cex.main=1.5, cex.lab=1.5)
...
conf[indd,1] <- min(prob_vec)
conf[indd,2] <- max(prob_vec)
conf[indd,3] <- mean(prob_vec)
forward_mean[indd,1] <- mean(nr_vec)*(nr_samples)
nr_optim[indd,1] <- min(nr_vec)
nr_optim[indd,2] <- max(nr_vec)
